...
export default async function InterpreterHome() {
  ...
  const { data: profileRow, error: profileError } = await supabase
    ...
    .select(
      "home_address,service_radius_miles,languages,total_completed,pro_bono_signed_at"
    )
    .eq("profile_id", user.id)
    .maybeSingle();

  if (profileError) {
    console.error("Interpreter profile load error:", profileError);

    throw new Error(
      `Could not load interpreter profile: ${profileError.message}`
    );
  }
  ...
  if (assignmentsError) {
    console.error(
      "Interpreter assignment count error:",
      assignmentsError
    );

    throw new Error(
      `Could not load assignments: ${assignmentsError.message}`
    );
  }
    ...
    <div className="space-y-8">
      ...
      <section className="grid sm:grid-cols-3 gap-4">
        <Stat
          ...
          value={`${profileRow?.service_radius_miles ?? 25} mi`}
        />

        <Stat
          label="Active assignments"
          value={String(assignmentsCount ?? 0)}
        />

        <Stat
          label="Pro bono completed"
          value={String(profileRow?.total_completed ?? 0)}
        />
      </section>
